        ...
        if (!$(this).hasClass('amoff')) {
            ...
            if (EMBEDLY !== 1 && URLEMBED !== 1) {
                if (file.indexOf('youtube.com/') !== -1 || file.indexOf('youtu.be') !== -1) {
                    ...
                    var eid = ematch.pop();
                    $(this).outerHTML("<div class=\"am_embed\"><iframe width=\"560\" height=\"315\" style=\"max-width="+MAXWIDTH+" max-height="+MAXHEIGHT+"\" src=\"//www.youtube.com/embed/"+eid+"\" frameborder=\"0\" allowfullscreen></iframe></div>");
                }
                if (file.indexOf('dailymotion.com/video/') !== -1) {
                    var ematch = file.match(/dailymotion\.com\/video\/([^_]+)/);
                    var eid = ematch.pop();
...
var amEmbed = function () {
    ...
    // Embed some audio an video attachments
    if (ATTACH == 1) {
